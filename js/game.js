...
const playerTurn = () => {
  gameText.innerText = `You've encountered a ${game.currentEnemy.name}, what will you do? OPTIONS: Attack or Use Item?`;
  getPlayerInput();
  if (userInput === 'attack') {
...
const fightBattle = () => {
  ...
  gameText.innerText = `You've spotted a ${game.currentEnemy.name} coming right for you! Time to fight!`;
  infoBox.updateGameText(`What will you do? OPTIONS: Attack or Use Item`);
  infoBox.getNextDialog(getPlayerInput);
  console.log(userInput);
  if (userInput === 'attack') {
    alert('It worked!');
  }
...
const mainGame = () => {
  ...
  infoBox.getNextDialog(fightBattle);
  while (game.play) {
    game.currentTurn = 'player';
